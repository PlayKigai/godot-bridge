...
const EXCLUDE_PATTERNS_CAP: usize = 256;
const EXCLUDE_PATTERN_BYTES_CAP: usize = 256;
const EXCLUDE_WILDCARDS_CAP: usize = 256;
...
const PROJECT_UNTRUSTED_KEYS: &[&str] = &[
    "godot_path",
    "project_dir",
    "lsp_port",
    "dap_port",
...
impl Settings {
    fn from_object(object: &Map) -> Result<Self, String> {
        let mut settings = Self {
            ...
            extra_args: string_array(object, "extra_args")?,
            exclude: string_array(object, "exclude")?,
            ..Self::default()
        };
...
fn string_array(object: &Map, key: &str) -> Result<Vec<String>, String> {
...
fn validate_settings(settings: &Settings) -> Result<(), String> {
    ...
    if settings.exclude.len() > EXCLUDE_PATTERNS_CAP {
        return Err(format!(
            "exclude has more than {EXCLUDE_PATTERNS_CAP} patterns"
        ));
    }
    if settings
        .exclude
        .iter()
        .any(|pattern| pattern.len() > EXCLUDE_PATTERN_BYTES_CAP)
    {
        return Err(format!(
            "exclude pattern longer than {EXCLUDE_PATTERN_BYTES_CAP} bytes"
        ));
    }
    let wildcards = settings
        .exclude
        .iter()
        .flat_map(|pattern| pattern.chars())
        .filter(|character| matches!(character, '*' | '?'))
        .count();
    if wildcards > EXCLUDE_WILDCARDS_CAP {
        return Err(format!(
            "exclude has more than {EXCLUDE_WILDCARDS_CAP} wildcards in total"
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn project_exclude_is_trusted() {
        let config_dir = TempDir::new().unwrap();
        let worktree = TempDir::new().unwrap();
        write_project_settings(
            worktree.path(),
            r#"{"lsp":{"godot":{"settings":{"exclude":["addons","*.test.gd"]}}}}"#,
        );
        let settings = load_file_settings(
            worktree.path(),
            Some(&user_settings_path_in(config_dir.path())),
        ...
        .unwrap();
        assert_eq!(settings.exclude, ["addons", "*.test.gd"]);
    }

    #[test]
    fn oversized_exclude_is_an_error() {
        let many = format!(r#"{{"exclude": [{}]}}"#, vec!["\"a\""; 257].join(","));
        let value: Value = crate::json::from_str(&many).unwrap();
        assert_eq!(
            parse_settings(&value).unwrap_err(),
            "invalid settings: exclude has more than 256 patterns"
        );
        let long = format!(r#"{{"exclude": ["{}"]}}"#, "b".repeat(257));
        let value: Value = crate::json::from_str(&long).unwrap();
        assert_eq!(
            parse_settings(&value).unwrap_err(),
            "invalid settings: exclude pattern longer than 256 bytes"
        );
        let stars = format!(
            r#"{{"exclude": [{}]}}"#,
            vec![format!("\"{}\"", "*a".repeat(100)); 11].join(",")
        );
        let value: Value = crate::json::from_str(&stars).unwrap();
        assert_eq!(
            parse_settings(&value).unwrap_err(),
            "invalid settings: exclude has more than 256 wildcards in total"
        );
    }

    #[test]
    fn non_string_exclude_entry_is_an_error() {
        let value: Value = crate::json::from_str(r#"{"exclude": ["ok", 7]}"#).unwrap();
        assert_eq!(
            parse_settings(&value).unwrap_err(),
            "invalid settings: exclude must be an array of strings"
        );
    }
