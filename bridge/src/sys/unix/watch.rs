//! inotify watch of the project tree, one watch per directory.
...
/// Stops the watch thread and closes the inotify descriptor when dropped.
...
impl WatcherState {
    fn watch_directory(&mut self, path: &Path) -> io::Result<()> {
        ...
        while let Some(directory) = directories.pop() {
            if directory_is_skipped(&directory, &self.project, &self.filter) {
                continue;
            }
    ...
    fn emit_event(
        ...
        if event.mask & libc::IN_ISDIR != 0 {
            if directory_is_skipped(&path, &self.project, &self.filter) {
                return true;
            }
