...
impl Mapper {
    ...
    fn emit_record(&mut self, record: &Record, publisher: &Publisher<'_>) -> bool {
        ...
        if directory_is_skipped(parent, &self.project, &self.filter) {
            return true;
        }
        if self.is_directory(&path, record.action) {
            if record.action == FILE_ACTION_MODIFIED
                || directory_is_skipped(&path, &self.project, &self.filter)
            {
                return true;
            }
...
mod tests {
    ...
    fn stop_is_prompt() {
        ...
        let watcher = watch_project_into(directory.path(), ScanFilter::default(), sender).unwrap();
    ...
    fn skipped_directory_is_silent() {
        ...
        let (sender, receiver) = mpsc::sync_channel(4096);
        let _watcher = watch_project_into(directory.path(), ScanFilter::default(), sender).unwrap();
