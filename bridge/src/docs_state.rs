...
pub fn scan_project_stream(
    ...
    while let Some(directory) = directories.pop() {
        ...
        for entry in entries {
            ...
            if file_type.is_dir() {
                if !directory_is_skipped(&path, &project, filter) {
            ...
            if path
                ...
                .is_ok_and(|relative| filter.exclude.is_excluded(relative, false))
            {
                continue;
            }
...
pub fn eligible_path(project: &Path, path: &Path, filter: &ScanFilter) -> bool {
    ...
    if filter.exclude.is_excluded(relative, false) {
        return false;
    }
    path.parent()
        .is_some_and(|parent| !directory_is_skipped(parent, &project, filter))
}
...
pub(crate) fn directory_is_skipped(path: &Path, project: &Path, filter: &ScanFilter) -> bool {
    let Ok(relative) = path.strip_prefix(project) else {
        return true;
    };
    relative.components().any(|component| {
        ...
        (!filter.diagnose_addons && name == OsStr::new("addons"))
            || name.to_string_lossy().starts_with('.')
    }) || filter.exclude.is_excluded(relative, true)
}
...
mod tests {
    ...
    fn scan_filters_project_diagnostics_files() {
        ...
        scan_project_stream(directory.path(), &ScanFilter::default(), |document| {
        ...
        scan_project_stream(directory.path(), &ScanFilter::new(true, &[]), |document| {
    ...
    #[test]
    fn directory_is_skipped_honours_exclude() {
        let project = Path::new("/project");
        let filter = ScanFilter::new(false, &["gen".to_owned()]);
        assert!(directory_is_skipped(
            &project.join("levels/gen"),
            project,
            &filter
        ));
        assert!(!directory_is_skipped(
    ...
    #[test]
    fn eligible_path_skips_excluded_directories_and_globs() {
        let directory = TempDir::new().unwrap();
        fs::create_dir(directory.path().join("skipme")).unwrap();
        fs::write(directory.path().join("skipme/inside.gd"), "x").unwrap();
        fs::write(directory.path().join("main.test.gd"), "x").unwrap();
        fs::write(directory.path().join("keep.gd"), "x").unwrap();
        let filter = ScanFilter::new(false, &["skipme".to_owned(), "*.test.gd".to_owned()]);
        assert!(!eligible_path(
            directory.path(),
            &directory.path().join("skipme/inside.gd"),
            &filter
        ));
        assert!(!eligible_path(
            directory.path(),
            &directory.path().join("main.test.gd"),
            &filter
        ));
        assert!(eligible_path(
            directory.path(),
            &directory.path().join("keep.gd"),
            &filter
        ));
    }

    #[test]
    fn scan_skips_excluded_directories_and_globs() {
        let directory = TempDir::new().unwrap();
        fs::create_dir(directory.path().join("skipme")).unwrap();
        fs::write(directory.path().join("skipme/inside.gd"), "x").unwrap();
        fs::write(directory.path().join("main.test.gd"), "x").unwrap();
        ...
        scan_project_stream(directory.path(), &filter, |document| {
            scanned.push(document);
            true
        });
        scanned.sort_by(|left, right| left.path.cmp(&right.path));
        assert_eq!(
            scanned.into_iter().map(|doc| doc.path).collect::<Vec<_>>(),
            vec![directory.path().join("keep.gd")]
        );
    }
