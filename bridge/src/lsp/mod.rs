...
pub fn run() -> Result<ExitCode> {
    ...
    let mut proxy = ProxyState::new(
        initialize.clone(),
        project.clone(),
        ScanFilter::new(settings.diagnose_addons, &settings.exclude),
        internal_sender,
        bulk_sender,
    );
...
fn rewrite_document_messages(
    ...
    match method {
        ...
        "textDocument/didClose" => {
            ...
            if reopen_from_disk
                && docs_state::eligible_path(&proxy.project, &key, &proxy.filter)
                && key.is_file()
            {
...
fn start_project_rescan(proxy: &mut ProxyState) {
    if proxy.rescan_active {
        return;
    }
...
fn start_project_scan(
    ...
    let _ = thread::Builder::new()
        ...
        .stack_size(256 * 1024)
        .spawn(move || {
            docs_state::scan_project_stream(&project, &filter, |document| {
    ...
    for change in changes {
        ...
        match change.kind {
            WatcherChangeKind::Created | WatcherChangeKind::Modified => {
                if !docs_state::eligible_path(&proxy.project, &path, &proxy.filter)
