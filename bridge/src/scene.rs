...
pub fn resolve_scene(project: &Path, file: &Path, exclude: &Exclude) -> Result<String, SceneError> {
    ...
    let adjacent = relative_file.with_extension("tscn");
    if project.join(&adjacent).is_file() && !exclude.is_excluded(&adjacent, false) {
        return Ok(res_path(&adjacent));
    }

    let expected_script = res_path(&relative_file);
    let mut scenes = Vec::new();
    collect_scenes(&project, &project, exclude, &mut scenes);
...
fn collect_scenes(
    ...
    directory: &Path,
    exclude: &Exclude,
    scenes: &mut Vec<(PathBuf, PathBuf)>,
) {
    ...
    for entry in entries.flatten() {
        ...
        if file_type.is_dir() {
            if !should_skip_directory(root, &path, exclude) {
                collect_scenes(root, &path, exclude, scenes);
            }
                ...
                .is_some_and(|extension| extension == "tscn")
        {
            if let Ok(relative) = path.strip_prefix(root) {
                if !exclude.is_excluded(relative, false) {
...
fn should_skip_directory(root: &Path, path: &Path, exclude: &Exclude) -> bool {
    ...
    name == "addons"
        ...
        || path
            .strip_prefix(root)
            .is_ok_and(|relative| exclude.is_excluded(relative, true))
}
...
mod tests {
    ...
    fn resolves_adjacent_stem() {
        ...
        assert_eq!(
            resolve_scene(directory.path(), &file, &Exclude::default()).unwrap(),
    ...
    #[test]
    fn skips_excluded_scene_files() {
        let directory = TempDir::new().unwrap();
        let file = script(directory.path(), "scripts/player.gd");
        fs::write(directory.path().join("scripts/player.tscn"), "").unwrap();
        fs::write(
            directory.path().join("scripts/other.tscn"),
            "[ext_resource path=\"res://scripts/player.gd\"]",
        )
        .unwrap();
        let exclude = Exclude::new(&["player.tscn".to_owned()]);
        assert_eq!(
            resolve_scene(directory.path(), &file, &exclude),
            Ok("res://scripts/other.tscn".to_owned())
        );
        let exclude = Exclude::new(&["*.tscn".to_owned()]);
        assert_eq!(
            resolve_scene(directory.path(), &file, &exclude),
            Err(SceneError::NoScene(file))
        );
    ...
    fn reports_no_scene() {
        ...
        let file = script(directory.path(), "scripts/player.gd");
        assert_eq!(
            resolve_scene(directory.path(), &file, &Exclude::default()),
            Err(SceneError::NoScene(file))
        );
    }
