...
| `exclude` | [] | Glob patterns kept out of the project-wide diagnostics and symbol scan and out of scene lookup. `*`, `?`, `**`. A bare name matches at any depth, a `/` or `./` before the name anchors the pattern to the project root except a leading `**/`, a trailing `/` matches directories only. No negation. At most 256 patterns of 256 bytes, 256 wildcards in total. |
...
Scene resolution for `rel/path.gd`: `rel/path.tscn` if it exists and no
`exclude` pattern matches it, else the first `.tscn` in lexicographic order
(skip `addons/`, hidden dirs, `exclude` matches, symlinks) whose
`[ext_resource ...]` line has `path="res://rel/path.gd"`.

## Project diagnostics and symbols

With `project_diagnostics`, after `initialized` reaches Godot and after each
recovery, every `.gd` (skip `addons/` unless `diagnose_addons`,
hidden dirs, `exclude` matches, symlinks) not already open gets `didOpen` from
disk as `Bridge`, 100 per batch, next batch when their diagnostics arrive or after
1 s. Over 2 MiB or invalid UTF-8: skipped.
