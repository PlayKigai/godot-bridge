...
`godot.*` in user or workspace settings; the bridge, Godot, project, port and
flag keys are read from user settings only. Only keys you set are sent to the
...
| `godot.dapPort` | 6006 | Debug port of the editor named by `lspPort`. Ignored otherwise. User settings only. |
| `godot.projectDiagnostics` | true | Diagnostics for all scripts, not only open ones. |
| `godot.diagnoseAddons` | false | Include `addons/`. |
| `godot.exclude` | `[]` | Glob patterns kept out of the project-wide diagnostics and symbol scan and out of scene lookup. A bare name matches at any depth, a `/` or `./` before the name anchors the pattern to the project root except a leading `**/`, a trailing `/` matches directories only. `*`, `?` and `**`, no negation. |
