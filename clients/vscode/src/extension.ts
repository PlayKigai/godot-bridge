...
const MACHINE_KEYS = ["godotPath", "projectDir", "extraArgs"];
const WORKSPACE_KEYS = ["lspPort", "dapPort", "projectDiagnostics", "diagnoseAddons", "startupTimeoutS"];
...
async function startClient(folder: vscode.WorkspaceFolder, output: vscode.OutputChannel): Promise<void> {
...
function restartClients(pick: () => vscode.WorkspaceFolder[], output: vscode.OutputChannel): Promise<void> {
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  const output = vscode.window.createOutputChannel("Godot Bridge");
